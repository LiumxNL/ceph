...
void AioImageWriteSame::assemble_extent(const ObjectExtent &object_extent,
                                    bufferlist *bl) {
  if (object_extent->off % m_data_len == 0
      m_image_ctx.object_cacher == NULL) {
    if (object_extent->off == m_off) {
      bl.append(m_buf, m_data_len);
    } else {
      bl.swap(data_bl);
    }
  } else {
    uint64_t left = object_extent->len;
...
void AioImageWriteSame::send_object_requests(
  ...
  if (m_image_ctx.object_cacher == NULL) {
    ...
    if (buf_off) {
      data_bl.append(buf + m_data_len - buf_off, buf_off);
      data_bl.append(buf, m_data_len - buf_off);
    } else {
      data_bl.append(buf, m_data_len);
    }
